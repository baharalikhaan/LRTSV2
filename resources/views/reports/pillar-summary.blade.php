...
@section('title', 'Pillar Summary Report - RTS')
...
<div class="no-print" style="margin-bottom:12px;">
    <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;">
        <form method="GET" action="{{ route('reports.pillar-summary') }}" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
            <div style="display:flex;align-items:center;gap:4px;">
                <label for="pillar_id" style="font-size:11px;font-weight:500;color:var(--ink-600,#4c4553);white-space:nowrap;">Pillar:</label>
                <select name="pillar_id" id="pillar_id" onchange="this.form.submit()" style="padding:4px 8px;border:1px solid var(--ink-200,#d8d6dc);border-radius:4px;font-size:11px;font-family:inherit;color:var(--ink-700,#38333e);background:#fff;min-width:160px;">
            ...
            @if(request('pillar_id') || request('grant_id'))
            <a href="{{ route('reports.pillar-summary') }}" style="font-size:10px;color:var(--ink-400,#8b8592);text-decoration:none;">&times; Clear</a>
            @endif
        </form>
...
<div class="a4-report-page">
    ...
    <div class="a4-report-header">
        ...
        <div class="a4-header-title-block">
            <div class="a4-report-title">Projects by Research Pillar</div>
            <div class="a4-report-desc">Aggregate report of projects grouped by research pillar</div>
        </div>
        ...
        <table class="a4-data-table">
            <thead>
                <tr>
                    ...
                    <th class="a4-cell-num">Research Call Active</th>
                    <th class="a4-cell-num">Students</th>
                    <th>LPI</th>
                </tr>
            </thead>
            <tbody>
                @forelse($pillarData as $row)
                    @php $firstRow = true; @endphp
                    @forelse($row['pillar']->projects as $project)
                    @php
                        $students = $project->students->count();
                        $lpi = $project->lpi;
                        $wStatus = $project->currentWorkflowStatus();
                    @endphp
                    <tr>
                        @if($firstRow)
                        <td class="a4-pillar-cell" rowspan="{{ $row['total_projects'] > 0 ? $row['total_projects'] : 1 }}">
                            <div class="a4-pillar-name">{{ $row['pillar']->pillar }}</div>
                            <div class="a4-pillar-count">{{ $row['total_projects'] }} project(s)</div>
                        </td>
                        ...
                        <td class="a4-cell-num">
                            <span class="a4-status {{ $wStatus === 'report' || $wStatus === 'graded' ? 'a4-status-active' : 'a4-status-inactive' }}">
                                {{ ucfirst($wStatus) }}
                            </span>
                        </td>
                        <td class="a4-cell-num">
                        ...
                        <td class="a4-cell-num">{{ $students }}</td>
                        <td>{{ $lpi ? $lpi->name : '—' }}</td>
                    </tr>
                    @empty
                    <tr>
                        <td class="a4-pillar-cell">
                            <div class="a4-pillar-name">{{ $row['pillar']->pillar }}</div>
                            <div class="a4-pillar-count">0 projects</div>
                        </td>
                        <td colspan="7" class="a4-cell-empty">No projects assigned to this pillar.</td>
                    </tr>
                    @endforelse
                @empty
                <tr>
                    <td colspan="8" class="a4-cell-empty">No pillars found.</td>
                </tr>
                @endforelse
            </tbody>
            <tfoot>
                @php
                    $tProjects = $pillarData->sum('total_projects');
                    $tStudents = $pillarData->sum('total_students');
                @endphp
                <tr>
                    <td colspan="4" class="a4-foot-label">Totals:</td>
                    <td class="a4-cell-num">{{ $tProjects }}</td>
                    <td class="a4-cell-num">{{ $pillarData->sum('registered_projects') }}</td>
                    <td class="a4-cell-num">{{ $tStudents }}</td>
                    <td></td>
                </tr>
            </tfoot>
        </table>
    ...
    <div class="a4-report-footer">
        ...
        <div class="a4-footer-text">
            ...
            <span>Pillar Summary Report</span>
            <span class="a4-footer-sep">|</span>
